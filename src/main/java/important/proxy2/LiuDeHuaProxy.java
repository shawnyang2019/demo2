...
public class LiuDeHuaProxy {
	 ...
	 * 
	 * @Method: getProxy
	 * @Description: 这个方法返回刘德华的代理对象：Person person =
	 *               LiuDeHuaProxy.getProxy();//得到一个代理对象
	 * @Anthor:孤傲苍狼
	 *
	 * @return 某个对象的代理对象
	 */
	public Person getProxy() {
		...
		// 代理对象的三个参数：代理对象的类加载器，目标对象所实现的接口数组，代理器
//		return (Person) Proxy.newProxyInstance(LiuDeHuaProxy.class.getClassLoader(), ldh.getClass().getInterfaces(),//方法的两种入参类型都可以
		return (Person) Proxy.newProxyInstance(LiuDeHuaProxy.class.getClassLoader(), LiuDeHua.class.getInterfaces(),
				new InvocationHandler() {
					 ...
					 */
					@Override
					// invoke 方法的三个入参：代理对象，代理对象所要实现的目标对象的方法，方法的参数？ 需要进一步验证
					public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
						// 如果调用的是代理对象的sing方法,再由代理对象去调目标对象的方法，并且在调取方法前后进行一些其他的方法增强
						if (method.getName().equals("sing")) {
							System.out.println("我是他的经纪人，要找他唱歌得先给十万块钱！！");
							// 已经给钱了，经纪人自己不会唱歌，就只能找刘德华去唱歌！
//							下面的invoke方法的第一个入参是sing方法所在的类的对象，第二个入参是sing方法的实参传递过来的值赋值给args
							return method.invoke(ldh, args); // 代理对象调用真实目标对象的sing方法去处理用户请求
						}
						// 如果调用的是代理对象的dance方法，如果调用的是代理对象的sing方法,再由代理对象去调目标对象的方法，并且在调取方法前后进行一些其他的方法增强
